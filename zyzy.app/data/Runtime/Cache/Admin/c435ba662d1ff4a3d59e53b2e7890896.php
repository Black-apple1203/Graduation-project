...
<div class="seltpye_y">
    ...
    <div class="downlist">
        <li url="<?php echo P(array('audit'=>'','audit_cn'=>''));?>">不限</li>
        ...
        <li url="<?php echo P(array('audit'=>'1','audit_cn'=>'审核通过'));?>">审核通过<span>(<?php echo ($count[3]); ?>)</span></li>
         <?php if($_GET['jobtype']!= 1 && $_GET['jobtype']!= ''): ?><li title="审核未通过" url="<?php echo P(array('audit'=>'3','audit_cn'=>'审核未通过'));?>">审核未通过<span>(<?php echo ($count[5]); ?>)</span></li><?php endif; ?>
    </div>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        <?php if($apply['Export']): ?>$("#ExPort_s").click(function () {
                var qsDialog = $(this).dialog({
                    title: '批量导出',
                    loading: true,
                    footer : false
                });
                var url = "<?php echo U('Export/Admin/ajax_export_jobs');?>";
                $.getJSON(url, function (result) {
                    qsDialog.setContent(result.data);
                });
            });
            ...
            $("#ExPort").click(function () {
                ...
                $("form[name=form1]").attr("action", "<?php echo U('Export/Admin/export_jobs');?>");
                $("form[name=form1]").submit();
            });<?php endif; ?>
    });
</script>
