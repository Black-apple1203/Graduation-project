    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="list_tr link_black">
            <div class="td" style=" width:40%;">
                <div class="left_padding striking link_blue">
                    ...
                    <a href="<?php echo url_rewrite('QS_explainshow',array('id'=>$vo['id'],'type_id'=>$vo['type_id']));?>" target="_blank" style="<?php if($vo['tit_color']): ?>color:<?php echo ($vo["tit_color"]); ?>;<?php endif; if($vo['tit_b'] > 0): ?>font-weight:bold<?php endif; ?>"><?php echo ($vo["title"]); if(C('qscms_subsite_open') == 1): ?>(<?php echo (($subsite_list[$vo['subsite_id']]['s_sitename'] != "")?($subsite_list[$vo['subsite_id']]['s_sitename']):"总站"); ?>)<?php endif; ?><!--分站标识--></a>
