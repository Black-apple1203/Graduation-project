...
class Image {
    ...
    static function buildString($string, $rgb=array(), $filename='', $type='png', $disturb=1, $border=true) {
        ...
        $border && @imagerectangle($im, 0, 0, $width - 1, $height - 1, $borderColor);
     ...
     * @return string
     */
    static function buildImageVerify($option,$verifyName='verify') {
        $config = array('mode'=>1,'type'=>'png','width'=>120,'height'=>60,'fontColor'=>'','size'=>25,'length'=>4,'bgColor'=>'','setpixel'=>25,'line'=>10);
        $config = array_merge($config,$option);
        $fontpath=  dirname(__FILE__).'/font/font.ttf';
        import('ORG.Util.String');
        $randval = String::randString($config['length'], $config['mode']);
        session($verifyName, md5($randval));
        $config['width'] = ($config['length'] * 10 + 10) > $config['width'] ? $config['length'] * 10 + 10 : $config['width'];
        if ($config['type'] != 'gif' && function_exists('imagecreatetruecolor')) {
            $im = imagecreatetruecolor($config['width'], $config['height']);
        } else {
            $im = imagecreate($config['width'], $config['height']);
        }
        if($config['bgColor']){
            $c = Image::hex2rgb($config['bgColor']);
        }else{
            $r = Array(225, 255, 255, 223);
            $g = Array(225, 236, 237, 255);
            $b = Array(225, 236, 166, 125);
            $key = mt_rand(0, 3);
            $c = array('r'=>$r[$key],'g'=>$g[$key],'b'=>$b[$key]);
        }
        $backColor = imagecolorallocate($im, $c['r'], $c['g'], $c['b']);    //背景色（随机）

        $borderColor = imagecolorallocate($im, 100, 100, 100);                    //边框色
        imagefilledrectangle($im, 0, 0, $config['width'] - 1, $config['height'] - 1, $backColor);
        imagerectangle($im, 0, 0, $config['width'] - 1, $config['height'] - 1, $borderColor);
        $stringColor = imagecolorallocate($im, mt_rand(0, 200), mt_rand(0, 120), mt_rand(0, 120));
        // 干扰
        for ($i = 0; $i < $config['line']; $i++) {
            imagearc($im, mt_rand(-10, $config['width']), mt_rand(-10, $config['height']), mt_rand(30, 300), mt_rand(20, 200), 55, 44, $stringColor);
        }
        for ($i = 0; $i < $config['setpixel']; $i++) {
            imagesetpixel($im, mt_rand(0, $config['width']), mt_rand(0, $config['height']), $stringColor);
        }
        if($config['fontColor']){
            $c = Image::hex2rgb($config['fontColor']);
        }else{
            $c = array('r'=>mt_rand(0, 255),'g'=>mt_rand(0, 255),'b'=>mt_rand(0, 255));
        }
        $whites = ImageColorAllocate($im, $c['r'], $c['g'], $c['b']);
        $y = $config['height'] - ($config['height'] - $config['size']) / 2;
        if(file_exists($fontpath)){
            $left = 0;
            for ($i = 0; $i < $config['length']; $i++) {
                $x = $config['size'] * $i + $left+10;
                imagettftext($im, $config['size'], mt_rand(10, 10), $x, $y, $whites, $fontpath, $randval{$i});
            }
        }else{
            for ($i = 0; $i < $config['length']; $i++) {
        ...
        Image::output($im, $config['type']);
    }
    // 中文验证码
    static function GBVerify($option,$verifyName='verify') {
        $config = array('mode'=>1,'type'=>'png','width'=>120,'height'=>60,'fontColor'=>'','fontface'=>'simhei.ttf','size'=>25,'length'=>4,'bgColor'=>'','setpixel'=>25,'line'=>10);
        $config = array_merge($config,$option);
        import('ORG.Util.String');
        $code = String::randString($config['length'], 4);
        $config['width'] = ($config['length'] * 45) > $config['width'] ? $config['length'] * 45 : $config['width'];
        ...
        $bkcolor = imagecolorallocate($im, $c['r'], $c['g'], $c['b']);    //背景色（随机）
        ...
        $whites = ImageColorAllocate($im, $c['r'], $c['g'], $c['b']);//字体色
        
        // 干扰
        for ($i = 0; $i < $config['line']; $i++) {
            $fontcolor = imagecolorallocate($im, mt_rand(0, 255), mt_rand(0, 255), mt_rand(0, 255));
            imagearc($im, mt_rand(-10, $config['width']), mt_rand(-10, $config['height']), mt_rand(30, 300), mt_rand(20, 200), 55, 44, $fontcolor);
        }
        for ($i = 0; $i < $config['setpixel']; $i++) {
            $fontcolor = imagecolorallocate($im, mt_rand(0, 255), mt_rand(0, 255), mt_rand(0, 255));
            imagesetpixel($im, mt_rand(0, $config['width']), mt_rand(0, $config['height']), $fontcolor);
        }
        if (!is_file($config['fontface'])) {
            $config['fontface'] = dirname(__FILE__) . "/font/" . $config['fontface'];
        }
        $fontsize = $config['size'] ? $config['size'] : mt_rand(16, 20);
        for ($i = 0; $i < $config['length']; $i++) {
            //$fontcolor = imagecolorallocate($im, mt_rand(0, 120), mt_rand(0, 120), mt_rand(0, 120)); //这样保证随机出来的颜色较深。
            $codex = String::msubstr($code, $i, 1);
            imagettftext($im, $fontsize, mt_rand(-60, 60), 40 * $i + 20, mt_rand(30, 35), $whites, $config['fontface'], $codex);
        }
        Image::output($im, $config['type']);
    }
