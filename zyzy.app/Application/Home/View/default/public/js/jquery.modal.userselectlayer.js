            ...
            $('.J_list_jobs').die().live('click', function() {
                if ($(this).hasClass('seledted')) {
                    ...
                    if (multipleValue) {
                        ...
                        //     }
                        // }
                        var jobSortTipHtml = '<div style="padding-bottom:18px;">您当前选择的职位类别是：<span style="color:#333">'+ title +'</span></div><div style="color:#333;padding-bottom:5px;">是否使用所选职位的招聘信息预设模板，<span style="color:#ff6600">使用后将覆盖原有内容！</span></div><div>模板信息包括：薪资待遇、学历要求、工作经验、职位描述等项目</div>';
                    doAutoFill();
                    function doAutoFill() {
                                ...
                                if (dataArr) {
                                    ...
                                    function fillContent() {
                                        ...
                                        if (dataArr['education']) {
                                            ...
                                            $('#education').closest('.select_input').find('.J_listitme_text').text(dataArr['education_cn']);
                                        }
                                        // 工作经验
                                        if (dataArr['experience']) {
                                            $('#experience').val(dataArr['experience']);
                                            $('#experience').closest('.select_input').find('.J_listitme_text').text(dataArr['experience_cn']);
                                        }
                                        //  招聘人数
                                        if (dataArr['amount']) {
